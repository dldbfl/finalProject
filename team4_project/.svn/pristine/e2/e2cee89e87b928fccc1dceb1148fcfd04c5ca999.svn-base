...
function enterkey(page,flag){
	if (window.event.keyCode == 13) {
		...
		if(flag ==''){
			jobForm.attr("action","list?page="+page+"&?perPageNum="+perPageNum+"&?searchType="+searchType+"&?keyword="+keyword)
		}
		
		if(flag == '등록'){
			jobForm.attr("action","registList?page="+page+"&?perPageNum="+perPageNum+"&?searchType="+searchType+"&?keyword="+keyword)
			
		}
		if(flag =='미완'){
			jobForm.attr("action","noRegistList?page="+page+"&?perPageNum="+perPageNum+"&?searchType="+searchType+"&?keyword="+keyword)
		}	
		jobForm.submit();
		flag==''
   }
...
function promp(consult_no,consult_type){
	...
	/* if(consult_type == '비밀글'){
		...
		location.href="checkPwd?consult_no="+consult_no
				
	}else{ */

		location.href="detail?consult_no="+consult_no
