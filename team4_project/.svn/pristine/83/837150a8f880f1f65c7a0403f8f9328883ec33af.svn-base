...
/**
 * 
 * @사용목적		:	이사장 소개 관리를 위한 컨트롤러
 * @작성자		    :	이누리
 * @작성날짜     	:	2020. 6. 18.
 * @마지막수정자	:	이누리
 * @마지막수정일	:	2020. 6. 18.오후 7:58:58
 * @see			    :	-
 *
 */
@Controller
@RequestMapping("/manager")
public class Chairman_introduceController {

	@Autowired
	private Chairman_introduceService chairman_introduceService;
		
	@RequestMapping(value="/chairman_introduce", method=RequestMethod.GET)
	public String chairman_introduce(Model model, Manager_boardVO manager_board) throws Exception {
		String url = "manager/chairman_introduce/chairman_introduce.page";
	...
	@Resource(name="manager_Chairman_Introduce_ImagePath")
	private String manager_Chairman_Introduce_ImagePath;
	
	
	@RequestMapping("/picture/chairman_introduce/{manager_board_no}")
	@ResponseBody
	public ResponseEntity<byte[]> sendPicture(@PathVariable("manager_board_no") int manager_board_no, HttpServletResponse response)
		...
		Manager_boardVO manager_board = (Manager_boardVO) chairman_introduceService.getChairman_introduce(manager_board_no).get("manager_board");
		...
		String savePath = manager_Chairman_Introduce_ImagePath + File.separator + manager_board.getManager_data_filename();
	...
	@RequestMapping(value="/chairman_introduce", method=RequestMethod.POST)
	public String modifyPOST(Model model, Manager_boardRequest manager_boardReq) throws Exception {
		String url = "manager/chairman_introduce/chairman_introduce_modify.page";
		...
		chairman_introduceService.modifyChairman_introduce(manager_board,4);
	...
	private String saveFile(MultipartFile file, String old_fileName, int introduce_no) throws Exception {		
		...
		File savePath = new File(manager_Chairman_Introduce_ImagePath /*+ File.separator + introduce_no*/);
