...
/**
 * 
 * @사용목적		:	교육과정 관리를 위한 컨트롤러
 * @작성자		    :	이누리
 * @작성날짜     	:	2020. 6. 18.
 * @마지막수정자	:	이누리
 * @마지막수정일	:	2020. 6. 18.오후 7:59:51
 * @see			    :	-
 *
 */
@Controller
@RequestMapping("/manager")
public class Education_curriculumController {

	@Autowired
	private Education_curriculumService education_curriculumService;
		
	@RequestMapping(value="/education_curriculum", method=RequestMethod.GET)
	public String education_curriculum(Model model, Manager_boardVO manager_board) throws Exception {
		String url = "manager/education_curriculum/education_curriculum.page";
	...
	@Resource(name="manager_Education_Curriculum_ImagePath")
	private String manager_Education_Curriculum_ImagePath;
	
	
	@RequestMapping("/picture/education_curriculum/{manager_board_no}")
	@ResponseBody
	public ResponseEntity<byte[]> sendPicture(@PathVariable("manager_board_no") int manager_board_no, HttpServletResponse response)
		...
		Manager_boardVO manager_board = (Manager_boardVO) education_curriculumService.getEducation_curriculum(manager_board_no).get("manager_board");
		...
		String savePath = manager_Education_Curriculum_ImagePath + File.separator + manager_board.getManager_data_filename();
	...
	@RequestMapping(value="/education_curriculum", method=RequestMethod.POST)
	public String modifyPOST(Model model, Manager_boardRequest manager_boardReq) throws Exception {
		String url = "manager/education_curriculum/education_curriculum_modify.page";
		...
		education_curriculumService.modifyEducation_curriculum(manager_board, 2);
	...
	private String saveFile(MultipartFile file, String old_fileName, int introduce_no) throws Exception {		
		...
		File savePath = new File(manager_Education_Curriculum_ImagePath /*+ File.separator + introduce_no*/);
