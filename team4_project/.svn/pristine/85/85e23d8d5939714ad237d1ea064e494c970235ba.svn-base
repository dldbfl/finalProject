...
<form role="form" id= "modifyform" name='modifyform' method=post enctype="multipart/form-data">
	...
	<div class="col-sm-12">
		 ...
		 <div class="col-sm-8" >
			<textarea readonly style="overflow: hidden;
								      font-size: 18px;
								      margin: 18px 0px 0px 14.1875px;
								      width: 700px;
								      height: 100px;
								      border: 1px solid sandybrown;" class="col-sm-12 enterprise_modifyset" name ="enterprise_coming">${manager_board.enterprise_coming}</textarea>     
		 </div>
		  <div class="col-sm-2" >
		 </div>
		...
		<%--  <input type="hidden" readonly class="col-sm-12 enterprise_modifyset" value="${enterprise_introduce.enterprise_introduce_writer}"> --%>                		   <hr>
		 <input type="hidden" readonly name="enterprise_introduce_no" class="col-sm-12 enterprise_modifyset" value="${manager_board.manager_board_no}"><hr>
	</div>
</form>
</div>
...
function Cansel_go(){
	...
	$('#picturePreView').css({'background-image':'url("<%=request.getContextPath()%>/manager/picture/enterprise_coming/${manager_board.manager_board_no}"'});
