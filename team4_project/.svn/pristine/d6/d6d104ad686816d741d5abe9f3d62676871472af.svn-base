...
/**
 * 
 * @사용목적		:	교육 목표 관리를 위한 컨트롤러
 * @작성자		    :	이누리
 * @작성날짜     	:	2020. 6. 18.
 * @마지막수정자	:	이누리
 * @마지막수정일	:	2020. 6. 18.오후 8:00:13
 * @see			    :	-
 *
 */
@Controller
@RequestMapping("/manager")
public class Education_purposeController {

	@Autowired
	private Education_purposeService education_purposeService;
		
	@RequestMapping(value="/education_purpose", method=RequestMethod.GET)
	public String education_purpose(Model model, Manager_boardVO manager_board) throws Exception {
		String url = "manager/education_purpose/education_purpose.page";
	...
	@Resource(name="manager_Education_Purpose_ImagePath")
	private String manager_Education_Purpose_ImagePath;
	
	
	@RequestMapping("/picture/education_purpose/{manager_board_no}")
	@ResponseBody
	public ResponseEntity<byte[]> sendPicture(@PathVariable("manager_board_no") int manager_board_no, HttpServletResponse response)
		...
		Manager_boardVO manager_board = (Manager_boardVO) education_purposeService.getEducation_purpose(manager_board_no).get("manager_board");
		...
		String savePath = manager_Education_Purpose_ImagePath + File.separator + manager_board.getManager_data_filename();
	...
	@RequestMapping(value="/education_purpose", method=RequestMethod.POST)
	public String modifyPOST(Model model, Manager_boardRequest manager_boardReq) throws Exception {
		String url = "manager/education_purpose/education_purpose_modify.page";
		...
		education_purposeService.modifyEducation_purpose(manager_board,3);
	...
	private String saveFile(MultipartFile file, String old_fileName, int introduce_no) throws Exception {		
		...
		File savePath = new File(manager_Education_Purpose_ImagePath /*+ File.separator + introduce_no*/);
