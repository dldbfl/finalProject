...
<script id="certificate-template" type="text/x-handlebars-template">
	<tr>
		...
		<td><i class="minusBtn fa fa-minus fa-2x" aria-hidden="true" onclick="certificateRemove(this);" ></i></td>
	</tr>
</script>
	<!--교수 경력사항현황 템플릿  -->
<script id="career-template" type="text/x-handlebars-template">
	<tr>
		...
		<td class="careerDate"><input type="date" class="careerFristDay{{index}}" onchange="pickStartDate(this,this.value);" >
			 ~ <input type="date" class="careerLastDay{{index}}" onchange="pickLastDate(this,this.value);"  >
...
<%----------------------------학생  js-------------------------------%>
	...
	$("input[name='experiencen']").change(function(){
		var experienceCheck=$("input[name='experiencen']").prop('checked');
		if(!experienceCheck){
