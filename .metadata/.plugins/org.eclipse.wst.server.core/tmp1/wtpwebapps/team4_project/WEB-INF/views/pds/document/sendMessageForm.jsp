...
<title>공유하기(쪽지발송)</title>
</head>
<body>
	<div class="wrap">
		<h3>쪽지 쓰기</h3>
		<form action="sendMessageShare" method="post">
					    ...
					    <div class="modal-content">
					      ...
					      <div class="td11">
								<select ondblclick="toDown()" id="baseList"  name="left" multiple size="7" class="bms-cust-sub-search-form-line2-in-checkarea">
									...
									<c:choose>
										<c:when test="${proList.member_status == 1 }">
											<option role="${proList.member_id }" value="status${proList.member_status }">${proList.member_name }(재직중)</option>
										</c:when>
										<c:when test="${proList.member_status == 2 }">
											<option role="${proList.member_id }" value="status${proList.member_status }">${proList.member_name }(가입대기)</option>
										</c:when>
										<c:otherwise>
											<option role="${proList.member_id }" value="status${proList.member_status }">${proList.member_name }(퇴사)</option>
										</c:otherwise>
									</c:choose>
								  	 	</c:forEach>
								</select>
	...
	function sendBtn(){
		...
		for(var i =0; i<addrleng; i++){
			...
			$("input[name='member["+i+"].member_email']").val(msgAddr);
